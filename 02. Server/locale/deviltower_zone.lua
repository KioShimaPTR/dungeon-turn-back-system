quest deviltower_zone begin
	state start begin
		...
		when 20348.chat."Kaldığın Yerden Devam Et" with d.find(pc.get_dungeonindex()) begin
			...
			if oldumu == 1 then
				say_npc()
				say("Lusifer'i zaten yok etmişsin.")
				return
			end
			if n == 0 then
				setskin(NOWINDOW)
		...
		when 20348.chat."Şeytan Kulesi " begin
			if pc.get_level() < 40 then
				...
				say("2. Kattan devam etmek isterseniz 50M yang gerekmektedir.")
				say_sari("Oyundan istemediğiniz halde çıkarsanız")
				say_sari("Kaldığınız yerden devam edebilirsiniz!")
				...
				elseif s == 2 then
					if pc.get_money() < 50000000 then
						say("Hay Aksi!")
						say("Envanterinde 50M yang olmadan seni içeri alamam.")
		...
		when 20074.chat."Üst katlar" or 20075.chat."Üst katlar" or 20076.chat."Üst katlar" with pc.in_dungeon() and pc.get_map_index() >= 660000 and pc.get_map_index() < 670000 begin
			if d.getf("check1") == 1 then
				say("")
				say("Çok kısa bir süre önce buradaydın?")
				return
			end
		if not npc.lock() then
			...
			say("Sadece 40. seviyeye ulaştıysan")
			...
			if pc.level >=40 then
				...
				say_title(mob_name(npc.get_race()))
				say("Üzgünüm sadece")
				say("40. seviyeye ulaşanları bir")
		...
		when kill with pc.in_dungeon() and pc.get_map_index() >= 660000 and pc.get_map_index() < 670000 and npc.get_race() == 8018 begin
			...
			if cont >= 4 then
				...
				d.setf("kesilenmetin", 0)
				d.spawn_mob(8019, 617, 676)
		...
		when kill with pc.in_dungeon() and pc.get_map_index() >= 660000 and pc.get_map_index() < 670000 and npc.get_race() == 8019 begin
			...
			d.setf("kesilenmetin",d.getf("kesilenmetin")+1)
			if d.getf("kesilenmetin") >= 150 then
				d.notice("Toplam 150 adet metin kesildi. Artık metin düşmeyecek!")
				return
			end
			d.notice("Kalan metin sayısı: "..(150-d.getf("kesilenmetin")))
			d.spawn_mob(8019, 617, 676)
		end
		...
		when 30302.use with pc.in_dungeon() and pc.get_map_index() >= 660000 and pc.get_map_index() < 670000 begin
			...
			if d.getf( "harita" ) != 0 then
				syschat("Nesneyi zaten kullandın!")
				pc.remove_item( "30302", 200)
				return
			end
	...
	when devil_jump_8.timer begin
		...
		d.notice("Bug kullanımı tespit edildi, dışarı gönderiliyorsun!")
		d.clear_regen()
		d.exit_all()
		end
	end
	...
	when kill with pc.in_dungeon() and pc.get_map_index() >= 660000 and pc.get_map_index() < 670000 and npc.get_race() == 1093 begin
		...
		notice_all(""..pc.get_name().." grubu, Lusifer'i öldürdü!")
	...
	--when 1095.kill with pc.in_dungeon() and pc.get_map_index() >= 660000 and pc.get_map_index() < 670000 begin
		...
		--d.setqf("can_refine", 1)
		--notice_all("".. pc.get_name() .." grubu, Mavi Ölüm'ü mağlup etti.")
